...
sap.ui.define([], function () {
  ...
  return {
    validatePostcode: function validatePostcode(countryModel, countryKey, postcode) {
    ...
    _getContext: function _getContext(countryKey, aCountries) {
      var result = "";
    ...
    validateSAPPostcode: function validateSAPPostcode(postcode, country, postcodeMax, postcodeRule) {
      ...
      switch (postcodeRule) {
        ...
        case 1:
          //remove blank spaces
          postcode = postcode.replace(/ /g, ''); //make sure postcode doesn't exceed maximum
        ...
        case 2:
          //remove blank spaces
          postcode = postcode.replace(/ /g, ''); //make sure postcode doesn't exceed maximum and doesnt contain alpha characters
        ...
        case 3:
          //remove blank spaces
          postcode = postcode.replace(/ /g, ''); //make sure postcode is exactly the correct length
        ...
        case 9:
          //remove blank spaces
          postcode = postcode.replace(/ /g, ''); //remove dashes

          postcode = postcode.replace(/-/g, ''); //canada specific rule 'ANA NAN' (could be one regex if I was any good at writing regexes - which im not)

          if (country === 'CA') {
            if (postcode.length != postcodeMax || !postcode[0].match(/[a-z]/i) || //alpha
            !postcode[1].match(/^\d$/) || //number
            ...
            !postcode[5].match(/^\d$/) //number
              ...
              if (postcode.length != postcodeMax || !postcode[0].match(/^\d$/) || //number
              !postcode[1].match(/^\d$/) || //number
              !postcode[2].match(/^\d$/) || //number
              !postcode[3].match(/^\d$/) || //number
              !postcode[4].match(/[a-z]/i) || //alpha
